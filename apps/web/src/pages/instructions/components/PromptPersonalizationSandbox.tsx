    ...
    <Card className="mt-6 bg-gray-900 border-gray-700 text-white">
      <CardHeader>
        <CardTitle className="flex items-center text-xl text-white">
          <TestTube2 className="w-6 h-6 mr-3 text-cyan-400" />
      ...
      <CardContent className="space-y-6">
        <div>
          <label className="font-semibold text-white mb-2 block">1. Choose a Persona (or write your own!)</label>
          <div className="flex flex-wrap gap-2 mb-3">
            {Object.keys(personas).map(name => (
              <Button key={name} variant="outline" size="sm" className="border-gray-600 hover:bg-gray-700" onClick={() => setSystemPrompt(personas[name])}>
                {name}
              </Button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <div className="md:col-span-2 space-y-2">
            <label htmlFor="user-input" className="font-semibold text-white">2. Ask a Question</label>
            <Input
              id="user-input"
              value={userInput}
          ...
          <Button onClick={handleGenerate} className="bg-cyan-600 hover:bg-cyan-500 text-white h-10">
        ...
        <AnimatePresence>
          {aiResponse && (
            <motion.div
              ...
              className="!mt-6">
              <label className="font-semibold text-white mb-2 block">3. AI's Response</label>
              <div className="p-4 bg-gray-800/70 border border-gray-700 rounded-lg text-gray-300 italic">
                {aiResponse}
              </div>
            </motion.div>
          )}
