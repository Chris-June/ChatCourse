  ...
  return (
    <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-center gap-4">
        <h3 className="text-xl font-bold text-white">RAG Workflow Visualizer</h3>
          ...
          <React.Fragment key={index}>
            <motion.div
              className={`flex flex-col items-center gap-2 p-3 rounded-lg border-2 transition-all duration-300 ${
                index === currentStep ? `${step.bgColor} border-current ${step.color}` : 'border-gray-700 text-gray-400'
              }`}
      ...
      <div className="bg-gray-900 rounded-lg p-4 min-h-[100px] flex items-center justify-center">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <p className="text-gray-300 mt-2 max-w-md mx-auto">{activeStep.description}</p>
          </motion.div>
        </AnimatePresence>
      </div>
    </div>
