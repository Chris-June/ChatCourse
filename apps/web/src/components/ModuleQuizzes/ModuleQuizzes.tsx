        ...
        <div className="mt-6">
          ...
          {questions.map((question, index) => (
                        <div key={index} className={`mb-4 p-4 border rounded-md ${getAnswerClasses(question, index)}`}>
                            ...
                            <p className={`text-gray-300 ${selectedAnswers[index] === question.correctAnswer ? 'text-green-400' : 'text-red-400'}`}>Your answer: {selectedAnswers[index] || 'Not answered'}</p>
                            <p className="text-green-400">Correct answer: {question.correctAnswer}</p>
                            <p className="text-sm text-gray-400 mt-2">Explanation: {question.explanation}</p>
            </div>
          ))}
        </div>
